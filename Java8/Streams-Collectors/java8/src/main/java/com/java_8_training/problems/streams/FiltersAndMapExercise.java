...
public class FiltersAndMapExercise {
    ...
    public static  List<Integer> findEvenNumbers() {
        ...
        evenNumbers = input.filter( (t) -> (t % 2 == 0)).collect(toList());
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(st -> st.equals("Jack")).collect(toList());
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = input.filter( p -> p.getName().equals(name) && p.getAge().equals(age)).collect(Collectors.toList());
    ...
    public static List<Integer> doubleNumbers() {
        ...
        doubleNumbers = input.map( t -> t*2).collect(toList());
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        //TODO #9
        Stream<Person> input = personList.stream();
        personOptional = input.filter(Person::isMale).findFirst();
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        int min = 0;
        Stream<Person> input = personList.stream();
        min = input.map(p -> p.getAge()).reduce(Integer.MAX_VALUE, Integer::min);
        return min;
    }
    ...
    public static double findAgeAverage(List<Person> personList){
        ...
        //TODO #13
        Stream<Person> input = personList.stream();
        sum = input.map( p -> p.getAge()).reduce(0, (su, x) -> su+x);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        persons = input.filter( p -> p.getName().startsWith("J")).map(p -> p.getNationality()).collect(Collectors.toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        Stream<Integer> input = someNumbers.stream();
        firstSquareDivisibleByThree = input.filter( i -> (i%3 == 0) && (Math.sqrt(i)==(int)Math.sqrt(i))).findFirst();
    ...
    public static int sumStringLength(){
        ...
        sum = input.map(p -> p.length()).reduce(0,(acc,x) -> acc+x);
        return sum;
    }
    ...
    public static  int sumSalary(List<Person> personList){
        ...
        //TODO #17
        Stream<Person> input =personList.stream();
        sum = input.mapToInt(Person::getSalary).sum();
        return sum;
    }
